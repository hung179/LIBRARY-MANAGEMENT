<template>
    <div class="max-w-2xl mx-auto p-6 bg-white shadow-md rounded-lg">
        <h2 class="text-2xl font-semibold mb-6 text-gray-800">Sửa sách</h2>
        <form @submit.prevent="submitForm">

            <div class="mb-4">
                <label class="block text-gray-700 font-medium mb-2" for="idBook">Mã định danh sách</label>
            ...
            <div class="mb-4">
                <label class="block text-gray-700 font-medium mb-2" for="remaining">Số Lượng Sách</label>
...
onMounted(() => {
    ...
    .then((newBook) =>{
        book.value = {
        idBook: newBook[0].idBook,
        bookName: newBook[0].bookName,
        image: newBook[0].image,
        author: newBook[0].author,
        price:  newBook[0].price,
        remaining: newBook[0].remaining,
        publicationYear: newBook[0].publicationYear,
        publisher: {
            name: newBook[0].publisher.name,
            address: newBook[0].publisher.address
        }
    };
    })
})
...
const submitForm = () => {
    Book.updateBooks(bookId ,book.value)

    alert('Sách đã được sửa thành công!');
    // Reset form
    router.push('/admin')
};
</script>
